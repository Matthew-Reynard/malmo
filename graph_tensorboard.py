...
# Avg time
with open("run_.-tag-average_time.csv", 'r') as csvfile:
...
# Avg score
with open("run_.-tag-score.csv", 'r') as csvfile:
	matrixreader = csv.reader(csvfile, delimiter=',')
	score = []
	episode = []
	for i, row in enumerate(matrixreader):
...
# Epsilon
with open("run_.-tag-epsilon.csv", 'r') as csvfile:
	matrixreader = csv.reader(csvfile, delimiter=',')
	epsilon = []
	episode1 = []
	for i, row in enumerate(matrixreader):
...
ax1.set_ylabel('average episode time (s)', color=color)
ax1.plot(episode, score, color=color)
ax1.tick_params(axis='y', labelcolor=color)

ax2 = ax1.twiny().twinx()  # instantiate a second axes that shares the same x and y axis
